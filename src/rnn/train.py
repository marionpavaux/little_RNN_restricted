...
def train_epoch(net: nn.Module, device: torch.device, dataloader: DataLoader, criterion, optimizer, beta1: float=None, beta2: float=None, beta_FR: float=None)->float:
    net.train()
    batch_losses = []
...
def test_epoch(net: nn.Module, device: torch.device, dataloader: DataLoader, criterion, beta1: float=None, beta2: float=None, beta_FR: float=None)->float:
    net.eval()
    batch_losses = []
    for (input_batch, label_batch) in dataloader: 
        input_batch = torch.fliplr(torch.rot90(input_batch, k=-1)).to(device, non_blocking=True)
