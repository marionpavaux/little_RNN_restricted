...
def plot_amp_stimvsresponse(
    stim_features: pd.DataFrame,
    emg_array: np.ndarray,
    amplitudes_range: list,
    label_stim_features: pd.DataFrame = None,
    label_emg_array: np.ndarray = None,
    norm: str = "mean",
    other_path: str = None,
) -> None:
    """
    ...
    if label_stim_features is not None:
        label_actual_amplitudes = np.unique(label_stim_features["Amplitude"])
    ...
    muscle_perc = (
        100 * np.nanmean(np.abs(emg_array), axis=1) if norm == "mean" else 100 * np.nanmax(np.abs(emg_array), axis=1)
    )
    if label_stim_features is not None:
        label_muscle_perc = (
            100 * np.nanmean(np.abs(label_emg_array), axis=1)
            if norm == "mean"
            else 100 * np.nanmax(np.abs(label_emg_array), axis=1)
        )

    fig = plt.figure(figsize=(30, 10))
    spec = gridspec.GridSpec(
        m.ceil(len(data.MUSCLES) / 4), 1 + 4 + 1, width_ratios=[0.166, 0.167, 0.167, 0.167, 0.167, 0.166]
    )
    axs = []
    ...
    for i in range(m.ceil(len(data.MUSCLES) / 4)):
        for j in range(4):
            ...
            if i * (len(data.MUSCLES) // 4 - 1) + j // 2 < len(data.MUSCLES) // 2:
                if label_emg_array is not None:
                    axs[i, j].plot(
                        ...
                        label_muscle_perc[
                            :,
                            (
                                i * (len(data.MUSCLES) // 4 - 1)
                                + j // 2
                                + (len(data.MUSCLES) // 2 + len(data.MUSCLES) % 2) * (j % 2)
                            ),
                        ],
                        "X",
                        color="#ffc9c5",
                ...
                axs[i, j].set_title(
                    f"{data.MUSCLES[(i*(len(data.MUSCLES)//4-1) + j//2 + (len(data.MUSCLES)//2 + len(data.MUSCLES)%2)*(j%2))]}",
                    fontsize=27,
                    loc="right",
                )
                axs[-1, j].set_xlabel("Amplitude of stimulation (mA)", fontsize="xx-large")
            else:
                axs[i, j].tick_params(axis="both", which="both", left=False, bottom=False)
            axs[i, j].tick_params(axis="both", which="both", labelleft=False, labelbottom=False)
...
def plot_cmap(ax: matplotlib.axes.Axes, amplitudes: list, cmap: list, min_amp: int = 0, max_amp: int = 7) -> None:
    ...
    ax.set_xticks(
        np.linspace(0, len(amplitudes[i_min_amp : i_max_amp + 1]) - 1, len(amplitudes[i_min_amp : i_max_amp + 1])),
        labels=amplitudes[i_min_amp : i_max_amp + 1],
        fontsize="xx-large",
    )
...
def plot_bars(
    ...
    spec = gridspec.GridSpec(
        len(data.MUSCLES) // 2 + 2, 3, width_ratios=[0.17, 0.415, 0.415], height_ratios=muscle_ratios
    )
    axs = []
    ...
    for i_muscle in range(len(data.MUSCLES) // 2):
        for direction in [0, 1]:
            ...
            for i_amp, amplitude in enumerate(actual_amplitudes):
                ...
                if direction == 0:
                    bar_plot_left = axs[i_muscle, direction].barh(
                        y_pos,
                        muscle_perc_left[i_amp::n_amplitudes, i_muscle],
                        color=CMAP_BAR[i_actual_amp],
                        zorder=len(actual_amplitudes) - i_amp,
                    )
                    # axs[i_muscle,direction].bar_label(bar_plot_left, fmt='%.1f', fontsize='xx-small', padding=-10)

                else:
                    bar_plot_right = axs[i_muscle, direction].barh(
                        y_pos,
                        muscle_perc_right[i_amp::n_amplitudes, i_muscle],
                        color=CMAP_BAR[i_actual_amp],
                        zorder=len(actual_amplitudes) - i_amp,
                    )  # label=f"{amplitude} mA")
                    # axs[i_muscle,direction].bar_label(bar_plot_right, fmt='%.1f',  fontsize='xx-small') # padding=-18

            axs[i_muscle, direction].set_yticks(
                y_pos,
                labels=frequencies,
                ha="right" if direction == 0 else "left",
                x=1.04 if direction == 0 else -0.04,
                fontsize="15",
            )
            axs[i_muscle, direction].tick_params(
                axis="y", which="both", left=False, right=False, labelleft=direction == 1, labelright=direction == 0
            )
        ...
        for direction in [0, 1]:
            ...
            for i_amp, amplitude in enumerate(actual_amplitudes):
                ...
                if direction == 0:
                    bar_plot_left = axs[i_muscle, direction].barh(
                        y_pos,
                        si_array[i_amp::n_amplitudes, i_muscle],
                        color=CMAP_BAR_SI[i_actual_amp],
                        zorder=len(actual_amplitudes) - i_amp,
                    )
                    # axs[i_muscle,direction].bar_label(bar_plot_left, fmt='%.1f', fontsize='xx-small', padding=-10)
                    ...
                    bar_plot_right = axs[i_muscle, direction].barh(
                        y_pos,
                        si_array[i_amp::n_amplitudes, len(data.MUSCLES) // 2 + i_muscle],
                        color=CMAP_BAR_SI[i_actual_amp],
                        zorder=len(actual_amplitudes) - i_amp,
                    )  # label=f"{amplitude} mA")
                    # axs[i_muscle,direction].bar_label(bar_plot_right, fmt='%.1f',  fontsize='xx-small') # padding=-18
...
def plot_amplitude_modulation(
    ...
    for i_muscle, muscle in enumerate(data.MUSCLES):
        axs[1].scatter(
            actual_frequencies, muscle_perc[:, i_muscle], label=muscle, marker="x", color=CMAP_MODULATION[i_muscle]
        )  # np.max(muscle_perc))
    ...
    for i_muscle, muscle in enumerate(data.MUSCLES[: len(data.MUSCLES) // 2]):
        for i_frequency, frequency in enumerate(frequencies):
            ...
            try:
                ...
                axs[i_muscle, i_actual_freq].plot(
                    time, emg_array[i_frequency, :, i_muscle], CMAP_EMGS[i_muscle], linewidth=5
                )
                if muscle[:1] != "TA":
                    axs[i_muscle, i_actual_freq].plot(
                        time,
                        emg_array[i_frequency, :, len(data.MUSCLES) // 2 + i_muscle],
                        CMAP_EMGS[len(data.MUSCLES) // 2 + i_muscle],
                        linewidth=5,
                    )
            except:
                None
